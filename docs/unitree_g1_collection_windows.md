# 宇树 G1 · 数据采集与回放（Windows）

本文说明宇树 G1 在比赛场景中的相机配置、Pico 遥操作、按钮脚本化数据采集、LeRobot 数据回放与数据格式。本文命令面向 **Windows 11 + CMD**。
...
请先按仓库根目录 [README](../README.md) 的 Windows 11 部分完成环境安装，并确认已在 OrcaLab 资产库中订阅 `Binjiang_Competition_2026` 与 `g1_pick`。

后续命令均在运行 OrcaLab 的主机上执行。先进入仓库根目录，再在 CMD 中执行：
...
示例命令中的数据集统一写入 `%USERPROFILE%\binjiang_datasets`。可以换成其它可写目录，但不要在同一目录中混入不同机器人、不同 state/action schema 或不同相机组合的数据。
...
先打开场景 `Binjiang_Competition_2026`，再加载 `g1_pick_buttons.json` 布局，确认当前 CMD 已进入 `unitree_g1` 目录并手动启动仿真，然后执行：
...
python g1_pick_osc_collection_tele_lerobot.py ^
    ...
    --agent_name g1_pick ^
    --lerobot_out "%USERPROFILE%\binjiang_datasets\g1_osc" ^
...
`set OMP_NUM_THREADS=1` 用于限制底层数值库的线程数，减少仿真、相机采集和视频编码之间的 CPU 争用。CMD 多行命令使用 `^` 作为续行符，`^` 后不要添加空格。
...
python g1_pick_osc_collection_scripted_lerobot.py ^
    ...
    --lerobot_out "%USERPROFILE%\binjiang_datasets\g1_osc_scripted" ^
...
带 `task` 的每个路点文件会单独生成 episode，脚本在该集开始前写入对应 prompt；同一命令中的路点 YAML 必须全部带 `task` 或全部不带，否则脚本会拒绝启动。上面的命令设置 `--num_episodes 1`，因此会依次保存按压、拨杆共 2 个 episode；设置为 `2` 时，每个任务重复 2 集，共保存 4 集。按钮任务不需要再传入 `--task`。相对路径仍以脚本目录为基准。
...
```cmd
set OMP_NUM_THREADS=1
python g1_pick_waypoint_mark.py ^
    --task_config example.yaml ^
    --agent_name g1_pick ^
    --button_name 拨杆1 ^
    --file_stem toggle ^
...
请先打开场景 `Binjiang_Competition_2026`，加载 `g1_pick_buttons.json` 布局，确认当前 CMD 已进入 `unitree_g1` 目录并手动启动仿真，然后执行：

```cmd
set OMP_NUM_THREADS=1
python g1_pick_osc_replay_lerobot.py ^
    --dataset_dir "%USERPROFILE%\binjiang_datasets\g1_osc_scripted" ^
    --task_config example.yaml ^
    --agent_name g1_pick ^
    --joint_strip on ^
    --strip_col off ^
...
- 已按根目录 README 的 Windows 11 部分安装运行环境，并激活 `orcalab_lerobot`。
...
- 数据集输出目录可写且磁盘空间充足。
- 按钮路点位于 `my_waypoint_button\marked\` 目录。
...
**现象**：脚本化采集找不到路点文件。 **处理**：相对路径以 `src\examples\dataCollection\unitree_g1` 为基准，按钮任务使用 `my_waypoint_button\marked\文件名.yaml`，多个文件之间只用逗号分隔。

**现象**：使用 `--resume` 时拒绝续写。 **处理**：检查旧数据集的 state/action feature 和相机键是否与当前命令一致。不要向不同 schema 或不同相机组合的数据集续写。

**现象**：视频编码失败或报找不到 `av1_nvenc`。 **处理**：确认 NVIDIA GPU 和驱动支持 AV1 NVENC，并确认当前 Windows 环境中的 PyAV/FFmpeg 支持该编码器。

**现象**：脚本报模块找不到。 **处理**：确认已激活 `orcalab_lerobot`，并按仓库根目录 README 的 Windows 11 安装步骤重新安装运行依赖。
